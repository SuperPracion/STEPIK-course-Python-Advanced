...
def generate_passwords(count, lenght):
    ...
    while len(passwords) != count:
        ...
        if sum([1 for item in temp_pass if item in ['l', 'I', '1', '0', 'o', 'O']]) == 0 and \
                sum([1 for item in temp_pass if item in string.digits]) > 0 and \
                sum([1 for item in temp_pass if item in string.ascii_uppercase]) > 0 and \
                sum([1 for item in temp_pass if item in string.ascii_lowercase]) > 0:
